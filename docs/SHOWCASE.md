...
Eino RAG Agent 是一个用 Go + Eino 构建的企业知识中台 / RAG 能力服务，支持向量知识库、LLM 编译 Wiki、Agentic 工具调用、GraphRAG、Code Search、MCP Export 和 React 管理前端。
...
4. **工程化点**：多租户 JWT、异步导入队列、DocReader/MinerU、Reranker、缓存、Swagger、Docker Compose、CI。
...
> Eino RAG Agent：基于 Go + Eino 构建企业知识中台，支持 Pipeline RAG、Agentic ReAct、向量/Wiki 双知识库、GraphRAG、Code Search、MCP Export、多租户鉴权和 React 管理前端；实现文档导入、检索、引用、聊天和对外工具化闭环。
